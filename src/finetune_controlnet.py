...
def main(args):
    ...
    train_dataset = Diffusion_Finetune_Dataset(preprocess_func=preprocess_func, use_exo=True, split='train')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                controlnet_image = batch["original_pixel_values"].to(dtype=weight_dtype)
            ...
            logs = {"loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "epoch": epoch, "step":global_step}
                    ...
                    with torch.no_grad():
                        for bn in tqdm(range(len(batch['text'][:10])), desc="Generating train images"):
                            ...
                            edited_image = pipeline(
                                batch['text'][bn], original_image, num_inference_steps=20, generator=generator
                            ).images[0]
